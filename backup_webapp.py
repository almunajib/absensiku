...
def refresh_machine_stats(machine_id):
    ...
    try:
        ...
        firmware, sn, user_count, record_count = client.get_all_info()
        ...
        if firmware and sn:
            try: # Update the database with the new info
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE machines SET firmware = ?, sn = ?, user_count = ?, record_count = ? WHERE id = ?",
                    (firmware, sn, user_count, record_count, machine_id),
                )
                conn.commit()
                conn.close()
